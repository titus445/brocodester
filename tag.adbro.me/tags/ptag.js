var adbro_init_function = function(w, d, undefined) {
    ...
    config.preview = config.preview || w.location.hash.indexOf('#adbro_preview') === 0;
    ...
    config.functions.onInventoryClick = config.functions.onInventoryClick || function(img, div, data, link) {};
    config.functions.onInventoryEmpty = config.functions.onInventoryEmpty || function(img, div) {};
    config.functions.onAdvertisementClose = config.functions.onAdvertisementClose || function(div) {};
    config.functions.getUILanguage = config.functions.getUILanguage || function() {
        return (navigator.language || navigator.userLanguage).split('-')[0];
    };
    config.functions.getPageTitle = config.functions.getPageTitle || function() {
    ...
    config.functions.getPageTags = config.functions.getPageTags || function() {
        return [];
    };
    ...
    config.functions.getImageSize = config.functions.getImageSize || function(img) {
        return {
    ...
    config.functions.getPageOrigin = config.functions.getPageOrigin || function() {
        return w.location.origin;
    };
    config.functions.getPageUrl = config.functions.getPageUrl || function() {
        let l = w.location_cached = w.location_cached || JSON.parse(JSON.stringify(w.location));
        let searchString = '';
        if (l.search.indexOf('=') !== -1) {
            try {
                let params = l.search.replace(/^[?&\/]+/, '').split('&').reduce(function(res, curr) {
                    var p = curr.split('=', 2);
                    if (p.length == 2 && p[0] && p[1]) res[p[0]] = p[1];
                    return res;
                }, {});
                const WASTE_PARAMS = {
                        names: ['zarsrc', 'pcode', 'zdlink', 'zl3rd', 'preview', 'reco_id', 'mcapp', '_ga', 'pageId', 'broadcastId', 'gidzl', '_trms', 'crst', 'wrst', 'rec_ts', 'cpa_tid', 'dicbo', 'tblci', 'commentid', 'tag_from', 'dc_data', 'ui'],
                        prefixes: ['utm', 'ia_', 'io_', '_gl', 'vnn_', 'vn_', 'fb_', 'uc_', '__', 'ldtag_'],
                ...
                for (let key in params) {
                    ...
                    if (REPLACE_PARAMS.hasOwnProperty(key) && params[key] === REPLACE_PARAMS[key].from) {
                        params[key] = REPLACE_PARAMS[key].to;
                    }
                }
                params = Object.keys(params).map((k) => encodeURIComponent(k) + '=' + encodeURIComponent(params[k]));
                if (params.length > 0) {
    ...
    config.functions.getPreviewAdvertisement = config.functions.getPreviewAdvertisement || function() {
        return config.previewAdvertisementGuid || getHashParameterValue('adbro_preview');
    };
    const DOMAINS_BLACKLIST = ['unblock.club', 'hyperbyte.net', 'vipshares.xyz', 'hideip.co', 'opoint.com', 'sogou.com', 'proxysite.com', 'codio.io', 'javascripts.page', 'ngon.info', 'worldlingo.com', 'archive.org', 'bingj.com', 'diigo.com', 'doubleclick.net', 'lavanetwork.net', 'genieesspv.jp', 'crwdcntrl.net', 'userreport.com', 'gammaplatform.com', 'adpia.vn', 'tenmax.io', 'atdmt.com', 'adsrvr.org', 'admicro.vn', 'ads.home.vn', 'ads.vietbao.vn', 'advertnative.com', 'mgid.com', 'outbrainimg.com', 'doubleverify.com', 'bidswitch.net', 'ssp.yahoo.com', 'ssp.advertising.com', 'rubiconproject.com', '000webhostapp.com', 'factset.com', 'useinsider.com', 'gravatar.com', ],
        DOMAINS_AND_PATTERNS_BLACKLIST = DOMAINS_BLACKLIST.concat(['vpn', 'proxy', 'localhost', '.local', 'mstarcms', 'nginx.', 'cloudfront.net', 'google.', '.google', ]);
    ...
    const EXPERIMENT__KEY = 'adbro-experiment-id',
        EXPERIMENT__GETTERS = {
            'page': function() {
                if (window.hasOwnProperty(EXPERIMENT__KEY)) return window[EXPERIMENT__KEY];
            },
            'session': function() {
        ...
        EXPERIMENT__SETTERS = {
            ...
            'session': function(value) {
                if (value === undefined) return sessionStorage.removeItem(EXPERIMENT__KEY);
                sessionStorage.setItem(EXPERIMENT__KEY, value);
            },
            'user': function(value) {
                if (value === undefined) return localStorage.removeItem(EXPERIMENT__KEY);
    ...
    function getExperimentId(choices, scope) {
        if (choices === undefined) {
            return EXPERIMENT__GETTERS['page']() || EXPERIMENT__GETTERS['session']() || EXPERIMENT__GETTERS['user']();
        }
        if (scope === undefined) throw 'Experiment scope is required.';
        if (!EXPERIMENT__GETTERS.hasOwnProperty(scope)) throw 'Experiment scope is invalid.';
        let choice = parseInt(EXPERIMENT__GETTERS[scope]());
        if (choices.indexOf(choice) < 0) choice = null;
        if (choice == null) choice = choices[Math.floor(Math.random() * choices.length)];
        EXPERIMENT__SETTERS['page']() || EXPERIMENT__SETTERS['session']() || EXPERIMENT__SETTERS['user']();
    ...
    const HEADER__ASSESSOR_KEY = 'X-ADBRO-assessor',
        ...
        URL__PANEL_SCRIPT_HANDLEBARS = '//cdnjs.cloudflare.com/ajax/libs/handlebars.js/4.7.6/handlebars.min.js',
    ...
    function getAdvertisementFor(params, callback, onerror) {
        ...
        let requestUri = config.endpoint +
            'v2/advertising/slot/' + slot + '/advertisement/',
        ...
        if (config.preview) {
            ...
            requestUri = config.endpoint +
                'v2/advertising/advertisement/' + advertisement +
                '/preview/?r=' + generateRandomString();
            requestParams = REQUEST_PARAMS__DEFAULT;
        }
        loadJson(requestUri, requestParams, function(data, xhr) {
    ...
    function getTemplateForAdvertisement(data, callback, onerror) {
        let requestUri = config.endpoint +
            'v2/advertising/template/' + data.template.id +
            '/version/' + data.template.version + '/';
        loadJson(requestUri, REQUEST_PARAMS__ANONYMOUS, function(template) {
            callback(template);
        }, onerror);
    }
    ...
    function getRelativeElementScroll(el) {
        ...
        if (transform.startsWith('translate3d')) {
            ...
            transformScroll = translate3d.length > 2 ? (parseInt(translate3d[1]) || 0) : 0;
        } else if (transform.startsWith('translateY')) {
            let translateY = transform.split('(');
            transformScroll = translateY.length > 1 ? (parseInt(translateY[1]) || 0) : 0;
        }
    ...
    const ENGAGEMENT__SELECTOR = VIEWABILITY__SELECTOR + ', .adbro-takeover-replay',
        ...
        ENGAGEMENT__TYPES_DEFAULT = [ENGAGEMENT__TYPE_HOVER, ENGAGEMENT__TYPE_TOUCH];
    ...
    function getSlotForCode(code) {
        if (SLOTS_MAP.hasOwnProperty(code))
            return SLOTS_MAP[code];
        return adbro.slot;
    }
    ...
    function buildTrackingLink(code, type, link, redirect, blockReason) {
        ...
        let trackUri = config.endpoint +
            'v2/advertising/slot/' + getSlotForCode(code) + '/track/' +
            '?code=' + encodeURIComponent(code) + '&event=' + type;
        if (redirect)
            trackUri += '&redirect=true';
        if (blockReason)
            trackUri += '&blockReason=' + blockReason;
        let userGuid = getUserGuid();
        if (userGuid)
            trackUri += '&uid=' + userGuid;
        let experimentId = getExperimentId();
        if (experimentId)
            trackUri += '&experiment=' + experimentId;
        trackUri += '&r=' + generateRandomString();
        if (link !== undefined)
            trackUri += '&link=' + encodeURIComponentOnce(link);
        return trackUri;
    }
    const BIDSWITCH__TYPE = 'bidswitch',
        BIDSWITCH__DEFAULT_STYLE = 'display:none; border: none; width: 476px; height: 90px;',
    ...
    const TTD_DV__MATCH = 'DVP_TTD_1',
        ...
        TTD_DV__SPAN = '<span id="' + TTD_DV__SPAN_ID + '" style="position:absolute; display:block; top:0; left:0; right:0; bottom:0;"></span>';
    ...
    function setupTag(img, placeholder, data) {
        ...
        if (type === BIDSWITCH__TYPE) {
            ...
            if (position && position !== 'uib') console.warn('Video Tag only supports Under-Image placement.');
            params['position'] = 'uib';
    ...
    function setupMessageListener(img, placeholder, adData, getAdvertisementParams) {
        let messageListener = null;
        messageListener = function(e) {
            let msg = e.data;
            if (!msg.hasOwnProperty('type') || !msg.hasOwnProperty('adbro_guid')) return;
            ...
            let adTag = adData.tag || {},
                adsMap = adTag.map || (adTag.parameters || {}).map || adTag.parameters || {},
                mapped = adsMap[msg.adbro_guid] || adsMap['map.' + msg.adbro_guid];
            if (mapped) {
                getAdvertisementParams.guid = mapped;
            }
            getAdvertisementFor(getAdvertisementParams, function(data) {
                ...
                getTemplateForAdvertisement(data, function(template) {
                    renderAdvertisement(img, placeholder, data, template, click_url, default_url, replacement_url);
                    onInventoryImpression(img, placeholder, data);
                });
            }, function() {
                console.error('Error when getting advertisement data for:', msg.adbro_guid);
                onInventoryEmpty(img, placeholder);
            })
            e.source.postMessage({
                type: 'ADBRO_PTAG_READY'
            }, '*');
        };
        w.addEventListener('message', messageListener);
    }

    function renderAdvertisement(img, div, data, template, linkTracking, linkDefault, linkReplacement) {
        ...
        function wrapClose(block) {
            [].forEach.call(block.querySelectorAll('.adbro-close, [data-block-reason]'), function(close) {
                ...
                close.onclick = function() {
                    ...
                    if (blockReason && data.track) {
                        let trackUri = buildTrackingLink(data.track, 11, undefined, undefined, blockReason);
                        loadPixel(trackUri);
                    }
                    closeAdvertisement();
                };
        ...
        function wrapLinks(block) {
            [].forEach.call(block.querySelectorAll('a[data-track=true]'), function(el) {
                ...
                if (linkTracking && !linkReplacement) {
                    trackUri = linkTracking + encodeURIComponent(wrapAdbro(destinationUri));
                    if (linkTracking.indexOf('adsrvr.org') > 0) {
                        trackUri = wrapAdbro(linkTracking.split('&r=')[0] + '&r=' + encodeURIComponent(destinationUri));
                    }
                }
        ...
        function translate(block) {
            ...
            [].forEach.call(block.querySelectorAll('[' + attribute + ']'), function(el) {
        ...
        function evalScript(script, onsuccess, onerror) {
            try {
                w.eval('(function (' + Object.keys(evalArguments).join(',') + ') {' + script + '})').apply(undefined, Object.values(evalArguments));
        ...
        if (data.script) {
            evalScript(data.script, null, function(e) {
                console.warn('Advertisement script evaluation has failed with error:', e)
            });
        }
        if (config.preview) return;
        let viewabilityTargets = [div.querySelector(VIEWABILITY__SELECTOR) || div];
        if (satellite) viewabilityTargets.push(satellite);
        setViewabilityTimeout(viewabilityTargets, VIEWABILITY__15S__TIME, function() {
            let trackUri = buildTrackingLink(data.track, VIEWABILITY__15S__EVENT);
            loadPixel(trackUri);
            div.classList.remove(CLASS__ANIMATED);
        });
        setViewabilityTimeout(viewabilityTargets, VIEWABILITY__6S__TIME, function() {
            let trackUri = buildTrackingLink(data.track, VIEWABILITY__6S__EVENT);
            loadPixel(trackUri);
        });
        setViewabilityTimeout(viewabilityTargets, VIEWABILITY__1S__TIME, function() {
            let trackUri = buildTrackingLink(data.track, VIEWABILITY__1S__EVENT);
            loadPixel(trackUri);
        });
        let engagementTargets = Array.from(div.querySelectorAll(ENGAGEMENT__SELECTOR)) || [div];
        ...
        setEngagementTimeout(engagementTargets, ENGAGEMENT__TYPE_TOUCH__TIME, engagementTrack, [ENGAGEMENT__TYPE_TOUCH]);
        setEngagementTimeout(engagementTargets, ENGAGEMENT__TYPE_HOVER__TIME, engagementTrack, [ENGAGEMENT__TYPE_HOVER]);
    }
    ...
    function onInventoryClick(img, placeholder, data, link) {
        ...
        if (macro && new RegExp('^(https?:)?\\/\\/', 'i').test(macro)) loadPixel(macro);
        config.functions.onInventoryClick(img, placeholder, data);
    }
    const COOKIESYNC__INTERVAL = 24 * 60 * 60 * 1000,
        LOTAME__URL = 'https://c.ltmsphrcl.net/id?gdpr_applied=false&us_privacy=false',
        STORAGE__COOKIESYNC_TS_KEY = 'adbro-cookiesync-ts';

    function cookieSync() {
        let userGuid = getUserGuid();
        if (!userGuid) return;
        let lastSyncTime = parseInt(localStorage.getItem(STORAGE__COOKIESYNC_TS_KEY) || 0),
            now = Date.now();
        if (lastSyncTime + COOKIESYNC__INTERVAL > now) return;
        localStorage.setItem(STORAGE__COOKIESYNC_TS_KEY, now);
        let trackUri = config.endpoint +
            'v2/advertising/cookie_sync?uid=' + userGuid +
            '&r=' + generateRandomString();
        loadJson(LOTAME__URL, null, function(data) {
            if (data && data.hasOwnProperty('core_id') && data['core_id']) {
                trackUri += '&externalSystemId=lotame&externalUserId=' + data['core_id'];
            }
            loadPixel(trackUri);
        }, function() {
            loadPixel(trackUri);
        ...
        [].forEach.call(images, function(img, id) {
            ...
            function loadAdvertisement() {
                ...
                getAdvertisementFor(getAdvertisementParams, function(data) {
                    ...
                    if (data.hasOwnProperty('tag') && !(config.preview && data.tag.type === BIDSWITCH__TYPE)) {
                        let initTag = function() {
                            w.adbro_tag_lock = true;
                            setupTag(img, placeholder, data);
                            setupMessageListener(img, placeholder, data, getAdvertisementParams);
                        };
                    ...
                    console.error('Error when getting advertisement data for:', imageUrl);
                    onInventoryEmpty(img, placeholder);
                });
                trackResize(placeholder, img);
            ...
            function waitForImageApproachScreenDelegate() {
                ...
                if ((distance < 0 && scrollAcceleration > -distance) || distance === 0 || (distance > 0 && -scrollAcceleration > distance)) {
                    loadAdvertisement();
                    return;
                }
    ...
    adbro._render = function(params) {
        ...
        getAdvertisementFor(_params, function(data) {
            getTemplateForAdvertisement(data, function(template) {
                renderAdvertisement(null, params.placeholder, data, template, (params.tag || {}).click_url);
                if (params.buttons === false) {
                    [].forEach.call(params.placeholder.querySelectorAll('.adbro-icon, .adbro-close'), function(el) {
                        el.remove();
                    });
                }
            });
...
if (window['ptag']) {
    let adbro = window['adbro'] = window['adbro'] || {
            config: {}
        },
    ...
    let CommandsProcessor = {
        process: function() {
            ...
            if (!adbro_initialized && Object.keys(adbro.config).length > 0 && adbro.slot) {
        ...
        _render: function(args) {
            if (args.length !== 2) throw 'Render command requires exactly 2 arguments.';
            args = args[1];
